...
object ImpalaKitQueries {
  ...
  val interactiveQueries =
    Seq("q19", "q42", "q52", "q55").map(queriesMap)
  val reportingQueries = Seq("q3","q7","q27","q43", "q53").map(queriesMap)
  ...
  val impalaKitQueries = interactiveQueries ++ reportingQueries 
  val xenonQueries = Seq("q53").map(queriesMap)
}
